...
###########################################################################################
## Arquivo: Nó Controlador de Trajetória de Klancar (Meio Retângulo)
...
###########################################################################################
    ...
    def __init__(self):
        ...
        rospy.init_node('klancar_laser_half_rect')
        ...
        self.small_side = float(raw_input('Comprimento do Lado Menor [m]: '))
        self.long_side = float(raw_input('Comprimento do Lado Maior [m]: '))
    ...
    def generate_half_rect(self):

        """Geração do meio retângulo que deverá ser seguido."""

        # Número de pontos do lado menor

        N_small = int(self.small_side / (self.base_lin_speed * self.Ts)) + 1

        # Número de pontos do lado maior

        N_long = int(self.long_side / (self.base_lin_speed * self.Ts)) + 1
        ...
        for k in range(1, N_small+1):
            ...
            self.theta_ref_list[k] = 0.0

        # Trecho Maior
        for k in range(N_small+1, N):
    ...
    def write_file(self):            
        ...
        path += '/meio_retangulo/resultados/ensaio_%d.txt' % self.ensaio
        ...
        params_str  = '### Ensaio do Controlador Klancar com Laser (Meio Retângulo) ###\n\n'
        params_str += '# Parâmetros #\n\n'
        params_str += '# Número do Ensaio: %d\n' % self.ensaio
        params_str += '# Frequência: %.1f Hz\n' % self.frequency
        params_str += '# Lado Menor: %.2f m\n' % self.small_side
        params_str += '# Lado Maior: %.2f m\n' % self.long_side
...
if __name__ == '__main__':
    ...
    klancar_half_rect = KlancarHalfRect()

    # Execução do laço principal

    klancar_half_rect.spin()

    # Registro dos dados do ensaio em um arquivo

    klancar_half_rect.write_file()

    # Finalização

    klancar_half_rect.finalize()
